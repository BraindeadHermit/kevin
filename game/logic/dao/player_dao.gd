...
func create_new_player(game_id):
	...
	var result = await db.insert_row("player", new_player)
	
	return result
	
func get_player_by_match_id(match_id):
	
	var res = await db.query("SELECT * FROM player WHERE player.game_id = " + str(match_id) + ";")
	if res:
...
func set_player_lifes(lifes, game_id):
	...
	var res = await db.query("UPDATE player SET life = " + str(lifes) + " WHERE game_id = " + str(game_id) + ";")
